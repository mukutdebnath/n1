...
from models.quant_dorefa import weight_noise_fn
...
class Conv2d_mvm_function(Function):
    ...
    def forward(ctx, input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1, bit_slice=2, bit_stream=1, weight_bits=16, weight_bit_frac=-1, input_bits=16, input_bit_frac=-1, adc_bit=-1, acm_bits=16, acm_bit_frac=-1, tile_row=2, tile_col=2, xbmodel=None, xbmodel_weight_path=None):
        ...
        ## Weight quantization here' on weight_xbar
        noise_fn = weight_noise_fn(cfg.wnoise_gamma, cfg.bit_slice, 0)
        weight_xbar += noise_fn(weight_xbar)
        ...
        #testing purpose to optimize the ADC cost
        # filename='first_layer_weight_{}.txt'.format(XBAR_ROW_SIZE)
        # pdb.set_trace()
        # for i in range(xbars.shape[2]):
        #     #f.write('\n')
        #     with open(filename, 'a') as f:
        #         np.savetxt(f, xbars[0,0,i].cpu().numpy(), delimiter=',')
