...
#def mvm_tensor(flatten_input, flatten_input_sign, bias_addr, xbars, bit_slice, bit_stream, weight_bits, weight_bit_frac, input_bits, input_bit_frac, adc_bit, acm_bit, acm_bit_frac, device):   # version 2
    ...
    if bit_stream == 1:
        for i in range(bit_stream_num): # 16bit input
            ...
            output_analog = torch.clamp(output_analog, min=0, max=2**adc_bit-1)   # the dictionary to be added here.
  
            ## addding real adc quantization
            if (cfg.is_adc):
                output_analog = torch.sum(output_analog.unsqueeze(-1) >= cfg.adc_th.cuda(), dim=-1).float()
